...
class _CabInterCityServiceScreenState extends State<CabInterCityServiceScreen> {
  final CameraPosition _kInitialPosition = const CameraPosition(
      target: LatLng(19.018255973653343, 72.84793849278007), zoom: 11.0, tilt: 0, bearing: 0);
  ...
  getCurrentOrder() {
    if (MyAppState.currentUser != null) {
      if (MyAppState.currentUser!.inProgressOrderID != null) {
        ordersFuture =
            FireStoreUtils().getIntercityOrder(MyAppState.currentUser!.inProgressOrderID.toString());
                  ...
                  child: Padding(
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                          ...
                          color: isDarkMode(context) ? Colors.black87 : Colors.white),
                      child: Padding(
                                                                ...
                                                                onTap: () async {
                                                                  if (popularDestination[index].latitude !=
                                                                    ...
                                                                    List<get_cord_address.Placemark>
                                                                        placeMarks = await get_cord_address
                                                                    ...
                                                                    setDestinationMarker(LatLng(
                                                                        popularDestination[index].latitude ??
                                                                            0.0,
                                                                        popularDestination[index].longitude ??
                                                                            0.0));
                                                                  }
                  ...
                  Text(
                    symbol +
                        (selectedVehicleType == null
                            ? "0.0"
                            : getAmount(selectedVehicleType!).toStringAsFixed(decimal)),
                    textAlign: TextAlign.center,
                    style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600),
                  ),
                ],
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                        color: Colors.white,
                        border:
                            selectedTripOption == "One-way" ? Border.all(color: Color(COLOR_PRIMARY)) : null,
                        borderRadius: const BorderRadius.only(
    ...
    if (_cabOrderModel != null) {
      ...
      if (statusOfOrder == ORDER_STATUS_DRIVER_ACCEPTED ||
        ...
        await getDurationDistance(
                LatLng(_cabOrderModel!.sourceLocation.latitude, _cabOrderModel!.sourceLocation.longitude),
                                    ...
                                    children: [
                                      ...
                                      Text(
                                          _driverModel!.reviewsCount != 0
                                              ? (_driverModel!.reviewsSum / _driverModel!.reviewsCount)
                                                  .toStringAsFixed(1)
                                              : 0.toString(),
                                          style: const TextStyle(
                                            letterSpacing: 0.5,
                              ...
                              Expanded(
                                child: MaterialButton(
                                  onPressed: () async {
                                    ...
                                    User? customer =
                                        await FireStoreUtils.getCurrentUser(cabOrderModel.authorID);
  ...
  setDepartureMarker(LatLng departure) {
    setState(() {
      ...
      _controller!.animateCamera(CameraUpdate.newCameraPosition(
          CameraPosition(target: LatLng(departure.latitude, departure.longitude), zoom: 14)));
  ...
  getDirections() async {
    ...
    if (statusOfOrder == ORDER_STATUS_DRIVER_ACCEPTED) {
      ...
      PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
        ...
        PointLatLng(
            _cabOrderModel!.destinationLocation.latitude, _cabOrderModel!.destinationLocation.longitude),
